...
async function pollDeployStatus() {
  try {
    const resp = await fetch(`${MODAL_PREFIX}/deploy/status`);
    if (!resp.ok) return;
...
async function loadModels() {
  ...
  try {
    const resp = await fetch(`${MODAL_PREFIX}/models`);
    if (resp.status === 503) {
  ...
  for (const folder of FOLDERS) {
    ...
    for (const file of files) {
      ...
      delBtn.onclick = async () => {
        ...
        delBtn.textContent = "…";
        try {
          const r = await fetch(`${MODAL_PREFIX}/models/${file.folder ?? folder}/${encodeURIComponent(file.name)}`, { method: "DELETE" });
      ...
      if (!alreadyInjected) {
        injectBtn.onclick = async () => {
          ...
          try {
            const r = await fetch(`${MODAL_PREFIX}/models/inject`, {
              method: "POST",
...
function buildAuthPanel(container, onConnected) {
  ...
  connectBtn.onclick = async () => {
    ...
    try {
      const resp = await fetch(`${MODAL_PREFIX}/auth/setup`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
function buildMainPanel(container) {
  ...
  redeployBtn.onclick = async () => {
    ...
    try {
      await fetch(`${MODAL_PREFIX}/deploy`, { method: "POST" });
      startDeployPoll();
    } catch (e) {
  ...
  reimportKeyBtn.onclick = () => {
    ...
    buildAuthPanel(parentContainer, () => {
      parentContainer.innerHTML = "";
      buildMainPanel(parentContainer);
      startDeployPoll();
    });
  };
  ...
  gpuSelect.addEventListener("change", async () => {
    ...
    try {
      await fetch(`${MODAL_PREFIX}/config`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  hfSaveBtn.onclick = async () => {
    ...
    try {
      const r = await fetch(`${MODAL_PREFIX}/settings/hf-token`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  civSaveBtn.onclick = async () => {
    ...
    try {
      const r = await fetch(`${MODAL_PREFIX}/settings/civitai-token`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token }),
      });
  ...
  async function loadTokenStatuses() {
    try {
      const [hfResp, civResp] = await Promise.all([
        fetch(`${MODAL_PREFIX}/settings/hf-token`),
        fetch(`${MODAL_PREFIX}/settings/civitai-token`),
      ]);
  ...
  injectAllBtn.onclick = async () => {
    ...
    try {
      const injectResp = await fetch(`${MODAL_PREFIX}/models/inject`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  downloadAllBtn.onclick = async () => {
    ...
    try {
      const resp = await fetch(`${MODAL_PREFIX}/models/batch-install`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ items }),
      });
      ...
      if (data.status === "ok") {
        ...
        data.results.forEach((res, i) => {
          ...
          injectPromises.push(
            fetch(`${MODAL_PREFIX}/models/inject`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
...
function buildWorkflowSection() {
  ...
  const wfUrlInput = document.createElement("input");
  wfUrlInput.type = "text";
  wfUrlInput.placeholder = "Add custom node URL (https://github.com/...)";
  wfUrlInput.style.cssText = inputStyle();
  wfUrlInput.style.display = "none";
  section.appendChild(wfUrlInput);
  ...
  async function loadCurrentNodes() {
    try {
      const r = await fetch(`${MODAL_PREFIX}/workflow/nodes`);
      const data = await r.json();
      if (data.status === "ok") {
        pendingNodes = data.nodes;
        renderPackages(pendingNodes, []);
        wfUrlInput.style.display = "block";
  ...
  analyzeBtn.onclick = async () => {
    ...
    try {
      ...
      const r = await fetch(`${MODAL_PREFIX}/workflow/analyze`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  addUrlBtn.onclick = () => {
    const url = wfUrlInput.value.trim();
    if (!url) return;
  ...
  wfUrlInput.addEventListener("keydown", (e) => { if (e.key === "Enter") addUrlBtn.click(); });
  ...
  buildBtn.onclick = async () => {
    ...
    try {
      ...
      const r = await fetch(`${MODAL_PREFIX}/workflow/build`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
function _showOnboardingBanner() {
  ...
  openBtn.textContent = "Scroll to Setup ↑";
  openBtn.onclick = () => {
...
async function initDashboard() {
  ...
  try {
    const resp = await fetch(`${MODAL_PREFIX}/auth/status`);
    const data = await resp.json();
    if (data.connected) {
      if (mainSection) mainSection.style.display = "block";
      if (authSection) authSection.style.display = "none";
      buildMainPanel(mainSection);
      startDeployPoll();
    } else {
      if (authSection) authSection.style.display = "block";
      if (mainSection) mainSection.style.display = "none";
      buildAuthPanel(authSection, () => {
        if (authSection) authSection.style.display = "none";
        if (mainSection) mainSection.style.display = "block";
        buildMainPanel(mainSection);
        startDeployPoll();
      });
      setTimeout(() => _showOnboardingBanner(), 2000);
    }
  } catch (e) {
    if (authSection) authSection.style.display = "block";
    if (mainSection) mainSection.style.display = "none";
